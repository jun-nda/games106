...
public:
	...
	*/
	...
	vks::Buffer ssbo;
	VkDescriptorSet        descriptorSet;
	std::vector<glm::mat4> nodeMatrixs;
	int32_t nodeCount = 0;
	...
	*/
	...
	void loadNode(const tinygltf::Node& inputNode, const tinygltf::Model& input, VulkanglTFModel::Node* parent, uint32_t nodeIndex, std::vector<uint32_t>& indexBuffer, std::vector<VulkanglTFModel::Vertex>& vertexBuffer)
	{
		...
		if (node->name == "node_Turbine_Controller_-15354") {
			//std::cout << "yeah" << std::endl;
		}
		auto mat = getFirstNodeMatrix(node);
		node->initMat = mat;
		node->initInverseMatrix = glm::inverse(node->matrix);
	...
	glm::mat4 VulkanglTFModel::getFirstNodeMatrix(VulkanglTFModel::Node* node)
	{
	...
	void VulkanglTFModel::updateJoints(VulkanglTFModel::Node *node)
	{
		...
		{
			// node->matrix = ;
			// auto curMat = UpdateNodeMatrix(node->translation, node->rotation, node->scale);

			//node->matrix = getNodeMatrix(node);
			node->matrix = node->getLocalMatrix();
			//node->matrix = mat * node->initInverseMatrix;
		...
		nodeMatrixs.emplace_back(nodeMatrix);
	...
	*/
	...
	void drawNode(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout, VulkanglTFModel::Node* node)
	{
		if (node->mesh.primitives.size() > 0) {
			...
			//nodeMatrix *= node->initInverseMatrix;
			...
			for (size_t i = 0; i < sampler.inputs.size() - 1; i++)
			{
				...
				if ((animation.currentTime >= sampler.inputs[i]) && (animation.currentTime <= sampler.inputs[i + 1]))
				{
					...
					if (channel.path == "scale")
					{
					 ...
					 //auto curMat = UpdateNodeMatrix(channel.node);
					 //channel.node->matrix = curMat;
				}
			}
		}
		//std::cout << animation.currentTime / animation.end << std::endl;
	}

	
	glm::mat4 UpdateNodeMatrix(Node* node)
	{
		glm::mat4 model = glm::mat4(1.0f);

		glm::vec3 translation = node->translation;
		glm::quat quaternion = node->rotation;
...
public:
	...
	void loadglTFFile(std::string filename)
	{
		...
		if (fileLoaded) {
			...
			nullptr));
			VK_CHECK_RESULT(glTFModel.ssbo.map()); // 所以创建了Host-visible内存之后，内存还是device的，所以要map一下，生成一个cpu地址
	...
	void setupDescriptors()
	{
		...
		*/
		...
		std::vector<VkDescriptorPoolSize> poolSizes = {
			...
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, static_cast<uint32_t>(1)),

		};
		// One set for matrices and one per model image/texture
		const uint32_t maxSetCount = static_cast<uint32_t>(glTFModel.images.size()) + 1 + 1; // pool的size也要自己设置....服了
		...
		// Node matrices
		VkDescriptorSetAllocateInfo allocInfo2 = vks::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayouts.nodeMatrices, 1);
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo2, &glTFModel.descriptorSet));
		VkWriteDescriptorSet writeDescriptorSet2 = vks::initializers::writeDescriptorSet(glTFModel.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 0, &glTFModel.ssbo.descriptor);
		vkUpdateDescriptorSets(device, 1, &writeDescriptorSet2, 0, nullptr);
	...
	void preparePipelines()
	{
		...
		const std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
			...
			// 该顶点受哪个刚体关节影响
			vks::initializers::vertexInputAttributeDescription(0, 4, VK_FORMAT_R8_SINT, offsetof(VulkanglTFModel::Vertex, matIndex))
		};
	...
	virtual void render()
	{
		...
		if (!paused)
		{
			...
			memcpy(glTFModel.ssbo.mapped, &glTFModel.nodeMatrixs, sizeof(glTFModel.nodeMatrixs));
		}
	}
